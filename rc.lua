...
local f = io.open("/home/ivan/.config/awesome/autostart.lst", "r")
...
kbdcfg.layout = { { "us", "" , "US" }, { "ru", "" , "RU" },  { "ua", "" , "UA" } }  
...
-- {{{ Key bindings
   -- awful.key({ modkey,           }, key_O, awful.client.movetoscreenu),
   -- awful.key({ modkey,           }, key_Y, function () kbdcfg.switch()                 end),
globalkeys = awful.util.table.join(
    ...
    awful.key({            }, Caps, function () kbdcfg.switch()                 end),
    awful.key({ }, key_Print, function () awful.util.spawn("scrot -e 'mv $f ~/Картинки/screenshots 2>/dev/null'") end),
    awful.key({ }, "Shift" , key_Print, function () awful.util.spawn("scrot -s 'mv $f ~/Картинки/screenshots 2>/dev/null'") end),
